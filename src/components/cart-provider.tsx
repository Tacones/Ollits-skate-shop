...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo(() => ({
    ...
    total: lines.reduce((n, l) => n + getProductPrice(l.product) * l.quantity, 0),
    add: (product: Product) => setLines((current) => current.some((l) => l.product.id === product.id)
      ? current.map((l) => l.product.id === product.id ? { ...l, quantity: l.quantity + 1 } : l)
      : [...current, { product, quantity: 1 }]),
    remove: (id: string) => setLines((current) => current.filter((l) => l.product.id !== id)),
  }), [lines]);
